...
const signIn = event => {
  let usrEmail = $("#userSignIn")
    .val()
    .trim();
  let usrPassword = $("#user_password")
    .val()
    .trim();
  event.preventDefault();
  firebase
    .auth()
    .signInWithEmailAndPassword(usrEmail, usrPassword)
    .then(checkLogin(), window.location.replace(`userProfile.html?${uid}`))
...
function init() {
  $("#topCreateLink").on("click", function() {
    $createProfModal.css("display", "block");
  });
  $("#topSignInLink").on("click", function() {
    $signInModal.css("display", "block");
  });
